...
resource "azurerm_managed_application" "nva" {
  ...
  lifecycle {
    ignore_changes = [
      parameter_values,  # Ignore any changes in parameters
      tags["chkp-lz-create-date"]  # Continue ignoring this tag
    ]
  }
...
data "external" "update-routing-intent" {
  ...
  program = ["python3", "./modules/add-routing-intent.py", "${local.req_url}", "${local.req_body}", "${local.access_token}"]
}
